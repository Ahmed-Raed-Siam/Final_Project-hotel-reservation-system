...
class UserRoleController extends Controller
{
    ...
    {
        /*$users_roles = DB::table('role_users')
                    ->join('roles', 'role_users.role_id', '=', 'roles.id')->orderBy('role_users.id')->paginate(10);*/
//        return response()->view('dashboard.users.roles.index', ['users_roles' => $users_roles]);
//        return response()->view('dashboard.users.roles.index', compact("users_roles"));
//        $user = User::withoutTrashed()->where('id', 8)->first();
        // Get All Users Which  have roles
        /*        $user_roles_ids = []; // an empty array of stored roles & permission IDs*/
//        $users_roles = User::withoutTrashed()->whereHas('roles')->orderByDesc('updated_at')->paginate(10);
//        $users_roles = User::withoutTrashed()->whereHas('roles')->orderBy('updated_at','DESC')->paginate(10);
        $users_roles = User::withoutTrashed()->whereHas('roles')->paginate(10);
//        $relation = User::withoutTrashed()->whereHas('roles')->first();
        /*Get Roles For specific user*/
//        $relation = User::withoutTrashed()->whereHas('roles')->where('id', 9)->first();
        /*Test Roles arrray [] works or not*/
//        dd($users_roles, $relation->roles()->get()[4]->name, $relation->roles()->get());
//        dd($users_roles);
        return response()->view('dashboard.users.roles.index', compact("users_roles"));
    }
...
//        dd($id, $user, $user_roles);
    ...
    public function edit(int $id): Response
    {
        ...
        // User ID
//        $user = User::withoutTrashed()->whereHas('roles')->findOrFail($id);
        $user = User::withoutTrashed()->where('id', $id)->firstOrFail();
        // Get all roles from roles table
        $roles = Role::all();
//        $user_roles = RoleUser::where('user_id', $id)->first();
//        $user_roles = RoleUser::all()->where('user_id', $id);
        $user_roles = RoleUser::where('user_id', $id)->get();
//        $user_roles = DB::table('role_users')->where('user_id', $id)->get();
        // print all User Role Data
//        foreach ($user_roles as $key => $user_role) {
//            echo "<h1>id $user_role->id</h1>";
//            echo "<h1>User id $user_role->user_id</h1>";
//            echo "<h1>Role id $user_role->role_id</h1>";
//            echo "<hr>";
//        }
//        dd($user_roles);
//        dd($user, $user->id, $user_roles, $user_roles[0]->getUserName($id));
//        dd($user, $user->id, $user_roles, $user_roles[0]->id);
//        dd($user_roles->user_id);
